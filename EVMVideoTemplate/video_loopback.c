...
uint8 CbBuffer[IMG_HEIGHT*IMG_WIDTH>>1];
uint8 CrBuffer[IMG_HEIGHT*IMG_WIDTH>>1];
...
void processing_task(void) {
  ...
  memset(CbBuffer,0x80,sizeof(uint8)*IMG_WIDTH*IMG_HEIGHT>>1);
  memset(CrBuffer,0x80,sizeof(uint8)*IMG_WIDTH*IMG_HEIGHT>>1);
  ...
  while ( 1 ) {
	...
	// process YBufferIn
	dataProcessing(YBufferIn,YBufferOut,IMG_HEIGHT,IMG_WIDTH);

	/* merge the Y, Cb and Cr buffers to frame buffer */
	y_cb_cr_to_ycbcr(YBufferOut,CbBuffer,CrBuffer,(uint8*)frameBuffPtr->frame.frameBufferPtr,IMG_WIDTH,IMG_HEIGHT);
